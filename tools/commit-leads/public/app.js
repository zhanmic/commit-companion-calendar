...
function colorizeStatusInText(text) {
  ...
  return escaped.replace(
    /\b(new|identified|researched|contacted|replied|demo|disqualified|won|lost)\b/gi,
...
async function showDetail(id, opts = {}) {
  ...
  detailBody.innerHTML = `
    ...
    ${lead.draft_email ? `<div class="draft">${escapeHtml(lead.draft_email)}</div>` : '<p class="hint">No draft email yet — run Score.</p>'}
  `
